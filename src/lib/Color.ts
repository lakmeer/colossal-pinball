...
export default class Color extends Array {
  ...
  static random () {
    const c = randFrom(Object.keys(TW).filter(k => k !== 'white' && k !== 'black'));
    const t = randFrom(Object.keys(TW[c]));
    return Color.fromTw(`${c}-${t}`);
  }

  static fromByte (r:number, g:number, b:number) {
    return new Color(r/255, g/255, b/255, 1);
  }

  static fromTuple ([ r, g, b, a, ]:Tuple4) {
  ...
  static fromTw (name:string) {
    switch (name) {
      case 'white': return Color.fromHex(TW.white);
      case 'black': return Color.fromHex(TW.black);
      default:
        const [ c, tone ] = name.split('-');
        const hex = TW[c] ? TW[c][tone] : TW.black;
  ...
  static interactive () {
    return Color.fromTw('fuchsia-500');
  }

  static static () {
    return Color.fromTw('blue-500');
  }

  static zone () {
    return Color.fromTw('green-500').alpha(0.3);
  }
